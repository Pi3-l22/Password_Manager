...
class RegisterPage(ft.Column):
    def __init__(self, page):
        ...
        vc.generate_digit(4)
        self.code = vc.code
        vc.save(f"./assets/{self.code}.png")
        # 标题
        ...
        self.vf_code = ft.Row(
            [
                ...
                ft.Image(
                    src=f"/{self.code}.png",  # NOTE: 本地图片路径
                    width=125,
                    height=50,
                ),
            ],
    ...
    def register_click(self, e):
        if self.username_box.value == "" or self.password_box.value == "":
            ...
            if self.vf_code.controls[0].value != self.code:
                ...
                # 删除验证码图片
                import os
                if os.path.exists(f"./assets/{self.code}.png"):
                    os.remove(f"./assets/{self.code}.png")
                # 重新生成验证码
                vc = VFCode()
                vc.generate_digit(4)
                self.code = vc.code
...
# Web服务器启动命令
# nohup python3 -m main.py  > run.log 2>&1 &